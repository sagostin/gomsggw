...
type Carrier struct {
	...
	Username  string `gorm:"not null" json:"username"`    // e.g., Account SID for Twilio (plaintext)
...
func (gateway *Gateway) loadCarriers() error {
	...
	for _, carrier := range carriers {
		// Decrypt password (username is stored as plaintext)
...
func (gateway *Gateway) addCarrier(carrier *Carrier) error {
	// Encrypt password only (username is stored as plaintext)
	encryptedPassword, err := EncryptAES256(carrier.Password, gateway.EncryptionKey)
	if err != nil {
		return fmt.Errorf("failed to encrypt password: %w", err)
	}

	// Store the original username for handler initialization
	plaintextUsername := carrier.Username
	plaintextPassword := carrier.Password
